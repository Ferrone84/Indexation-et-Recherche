...
/**
 * Classe utilisée pour évaluer la performance d'un index vectoriel sur un jeu
 * de requêtes prédéfinies, pour lesquelles on connait la vérité terrain.
 */
public class RankingEvaluator extends AbstractEvaluator {
	/**
	 * Initialise un évaluateur vectoriel pour la vérité terrain spécifiée dans
	 * la configuration.
	 * 
	 ...
	 *             Problème lors de la lecture de la vérité terrain
	 */
	public RankingEvaluator() throws ParserConfigurationException,
	 ...
	 * paramètre {@code answers} est la liste des réponses <i>ordonnée</i>
	 * renvoyées par le moteur de recherche pour toutes ces requêtes. Le
	 * paramètre {@code k} permet de distinguer les document considérés comme
	 * pertinents par le moteur ({@code k} premiers) des non-pertinents.
	 * 
	 * @param answers
	 *            Liste de listes d'objets DocScore renvoyée par le moteur de
	 *            recherche pour les requêtes traitées.
	 * @param k
	 *            Seuil utilisé pour distinguer les documents considérs comme
	 *            pertinents des non-pertinents.
	 ...
	 */
	private List<Map<MeasureName, Float>> evaluateQueryAnswers(
			List<List<DocScore>> answers, int k) {
		List<List<Posting>> convAnswers = new ArrayList<List<Posting>>();

		// on convertit chaque réponse séparément
		for (List<DocScore> answer : answers) {
			...
			int i = 0;
			// on ne garde que les k premiers documents
		...
		// on applique ensuite la méthode existante
		List<Map<MeasureName, Float>> result = evaluateQueryAnswers(convAnswers);
		return result;
	}
	...
	// //////////////////////////////////////////////////
	/**
	 * Evalue le moteur de recherche spécifié avec la vérité terrain
	 * précédemment chargée, et enregistre et renvoie les performances.
	 * 
	 * @param engine
	 *            Moteur de recherche à évaluer.
	 * @return Liste de maps contenant les performances calculées pour les
	 *         requêtes. Chaque map correspond à des valeurs moyennes pour une
	 *         valeur de k donnée. Ces moyennes sont obtenues en considérant
	 *         toutes les requêtes d'évaluation.
	 * 
	 ...
	 *             Problème à l'enregistrement des performances.
	 */
	public List<Map<MeasureName, Float>> evaluateEngine(
			RankingQueryEngine engine) throws FileNotFoundException,
		...
		for (int k = 1; k <= docNbr; k++) {
			...
			List<Map<MeasureName, Float>> temp = evaluateQueryAnswers(answers,
					k);
