...
/**
 * Classe utilisée pour évaluer la performance d'un index booléen sur un jeu de
 * requêtes prédéfinies, pour lesquelles on connait la vérité terrain.
 */
public class BooleanEvaluator extends AbstractEvaluator {
	/**
	 * Initialise un évaluateur booléen pour la vérité terrain spécifiée dans la
	 * configuration.
	 * 
	 ...
	 *             Problème lors de la lecture de la vérité terrain
	 */
	public BooleanEvaluator() throws ParserConfigurationException,
	 ...
	 *             Problème à l'enregistrement des performances.
	 */
	public List<Map<MeasureName, Float>> evaluateEngine(AndQueryEngine engine)
		...
		for (String query : queries) {
			List<Posting> answer = engine.processQuery(query);
			answers.add(answer);
		}
		
		// on calcule les performances correspondant aux réponses
		List<Map<MeasureName, Float>> result = evaluateQueryAnswers(answers);
